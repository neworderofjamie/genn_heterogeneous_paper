...
class LIFInt : public NeuronModels::Base
{
public:
    DECLARE_SNIPPET(LIFInt);
    ...
    SET_THRESHOLD_CONDITION_CODE("RefracTime < 0 && VFloat >= Vthresh");
    ...
    SET_DERIVED_PARAMS({
        {"ExpTC", [](const ParamValues &pars, double dt){ return std::exp(-dt / pars.at("TauM").cast<double>()); }},
        {"Rmembrane", [](const ParamValues &pars, double){ return  pars.at("TauM").cast<double>() / pars.at("C").cast<double>(); }},
        {"TauRefracTimestep", [](const ParamValues &pars, double dt){ return (int)std::round(pars.at("TauRefrac").cast<double>() / dt); }}});

    SET_VARS({{"V", "int16_t"}, {"RefracTime", "int16_t"}});

    SET_NEEDS_AUTO_REFRACTORY(false);
};
IMPLEMENT_SNIPPET(LIFInt);
...
void modelDefinition(ModelSpec &model)
{
    ...
    auto *pop = model.addNeuronPopulation<NeuronModels::LIF>("Pop", 100000, lifParams, lifInit);
